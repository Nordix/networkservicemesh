...
func TestOvSNSCAndICMPLocal(t *testing.T) {
	if testing.Short() {
		t.Skip("Skip, please run without -short")
		return
	}

	testOvSNSCAndICMP(t, 2, "")
}

func TestOvSNSCAndICMPRemoteVXLAN(t *testing.T) {
	if testing.Short() {
		t.Skip("Skip, please run without -short")
		return
	}
...
func testOvSNSCAndICMP(t *testing.T, nodesCount int, remoteMechanism string) {
	...
	err = k8s.SetForwardingPlane(pods.EnvForwardingPlaneOvS)
	g.Expect(err).To(BeNil())
	...
	for i := 0; i < nodesCount; i++ {
		...
		cfg.ForwarderVariables = kubetest.DefaultForwarderVariables(pods.EnvForwardingPlaneOvS)
